...
class LoggerInterceptor extends Interceptor {
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    ...
    if (error) {
      if (err.type == DioExceptionType.badResponse) {
        ...
        _printBoxed(header: 'DioError ║ Status: ${err.response?.statusCode} ${err.response?.statusMessage} ║ Time: $diff ms', text: uri.toString());
  ...
  void _printResponseHeader(Response response, int responseTime) {
    ...
    _printBoxed(header: 'Response ║ $method ║ Status: ${response.statusCode} ${response.statusMessage}  ║ Time: $responseTime ms', text: uri.toString());
  }
      ...
      if (value is Map) {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          for (var i = 0; i < lines; ++i) {
            ...
            logPrint('║${_indent(tabs)} $multilineKey ${msg.substring(i * linWidth, math.min<int>(i * linWidth + linWidth, msg.length))}');
          }
        } else {
